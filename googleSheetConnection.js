      /* exported gapiLoaded */
            ...
            function maybeEnableButtons() {
              ...
              console.assert("Google Docs Libs loaded!");
        ...
        function loadTheSheetValuesForSheetCutText(){
          if(sheetCutTextsValues == null){
            gapi.client.sheets.spreadsheets.values.get({
              spreadsheetId: '1YfKAUdQ8EV06c4RThiVrrRNPdvoWtxtdYInQW-NP6SE',
              range: 'CutText', // Adjust the sheet name as needed
            }).then(function(response) {
              sheetCutTextsValues = response.result.values;
            }, function(error) {
              console.error('Error fetching sheet data:', error);
            });
          }
        ...
        function loadTheSheetValuesForSheetTopText(){
          if(sheetTopTextValues == null){
            gapi.client.sheets.spreadsheets.values.get({
              spreadsheetId: '1YfKAUdQ8EV06c4RThiVrrRNPdvoWtxtdYInQW-NP6SE',
              range: 'TopText', // Adjust the sheet name as needed
            }).then(function(response) {
              sheetTopTextValues = response.result.values;
            }, function(error) {
              console.error('Error fetching sheet data:', error);
            });
          }
        ...
        function findRowByText(searchText, inputSheetValues){
          ...
          if (inputSheetValues && inputSheetValues.length > 0) {
            for (var i = 0; i < inputSheetValues.length; i++) {
              ...
              if (inputSheetValues[i][0] === searchText) {
                console.log('Row found:', inputSheetValues[i]);
          ...
          if(desiredRowFound == true){
            return inputSheetValues[rowIndex];
          }
          else{
